...
class Adaboost(BaseLearner):
    ...
    def fit(self, factors_return_df, QN=5, L=100, tol=1e-6, fcycle=0):
        ...
        if mode == 'Regressor':  # 此时i为最后一轮训练的指标，输出时最后一轮的结果舍弃，因为无明显效果（其实可以不舍弃）
            self.model = BestFactors[:i], BestWs[:i], WeakRegressorWeights[:i], Qs, eps
        else:  # 对于Classifier问题的输出
    ...
    def predict(self, factors_df):
        ...
        stockCodes = []
        def RobustAdaboostPredict(factors_df, model, mode='Regressor'):
            if mode == 'Regressor':
            ...
            if mode == 'Regressor':
                ...
                Yp = Yps.sum(axis=1)
            return Yp
        Yp = RobustAdaboostPredict(factors_df, model, mode='Regressor')
