...
# 获取交易所可转债发行代码，限制评级aa+以上
result_list_sh_aaa = w.wset("newbondissueview",
                            "startdate=%s;enddate=%s;datetype=startdate;bondtype=convertiblebonds;dealmarket=sse;maingrade=all;maingrade=aaa" % (START_DATE, END_DATE)).Data[0]
result_list_sz_aaa = w.wset("newbondissueview",
                            "startdate=%s;enddate=%s;datetype=startdate;bondtype=convertiblebonds;dealmarket=szse;maingrade=all;maingrade=aaa" % (START_DATE, END_DATE)).Data[0]
result_list_sh_aap = w.wset("newbondissueview",
                            "startdate=%s;enddate=%s;datetype=startdate;bondtype=convertiblebonds;dealmarket=sse;maingrade=all;maingrade=aa+" % (START_DATE, END_DATE)).Data[0]
result_list_sz_aap = w.wset("newbondissueview",
                            "startdate=%s;enddate=%s;datetype=startdate;bondtype=convertiblebonds;dealmarket=szse;maingrade=all;maingrade=aa+" % (START_DATE, END_DATE)).Data[0]
conv_bonds = result_list_sh_aaa + result_list_sz_aaa + result_list_sh_aap + result_list_sz_aap
...
for conv_bond, issue_amount, issue_date, listing_date, underlying_code \
    ...
    underlying_buy_date = w.wss(conv_bond, "issue_announcement").Data[0][0].strftime('%Y-%m-%d')  # 正股买入日期
    underlying_buy_date = get_trading_date_from_now(underlying_buy_date, 0)
    underlying_sell_date = get_trading_date_from_now(issue_date, T2)  # 正股卖出日期
    if underlying_code is None or underlying_code[-2:] not in ['SZ', 'SH'] or \
            (datetime.strptime(underlying_buy_date, '%Y-%m-%d') >= datetime.strptime(underlying_sell_date, '%Y-%m-%d')):  # 可转债无正股代码信息或正股买入卖出信息不符，跳过
        continue
    # 获取正股走势变动信息
    underlying_price = [w.wsd(underlying_code, "open", underlying_buy_date, underlying_buy_date, "PriceAdj=T").Data[0][0],
                        w.wsd(underlying_code, "close", underlying_sell_date, underlying_sell_date, "PriceAdj=T").Data[0][0]]
    print(conv_bond, underlying_code, underlying_price, underlying_buy_date, underlying_sell_date)
