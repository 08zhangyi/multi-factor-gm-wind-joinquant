...
def OnStart(context) :
    ...
    context.long_callopt_level = 1        #-1，-2，-3分别表示虚1，虚2，虚3；1，2，3表示实1，实2，实3，0表示平值合约
    context.short_callopt_level = -1
    #登录交易账号
...
def OnMarketQuotationInitialEx(context, exchange,daynight):
    if exchange == 'SHFE' and daynight == 'day':
        ...
        if len(d) <= context.offset_day+2:
            context.long_callopt = GetAtmOptionContractByPos(context.code_main, "now", context.long_callopt_level, 0, 1)
            context.short_callopt = GetAtmOptionContractByPos(context.code_main, "now", context.short_callopt_level, 0, 1)
            context.optopenlist = [context.long_callopt, context.short_callopt]
        else:
            context.long_callopt = GetAtmOptionContractByPos(context.code_main, "now", context.long_callopt_level, 0, 0)
            context.short_callopt = GetAtmOptionContractByPos(context.code_main, "now", context.short_callopt_level, 0, 0)
            context.optopenlist = [context.long_callopt, context.short_callopt]
        print(GetContractInfo(context.long_callopt)['名称'], GetContractInfo(context.short_callopt)['名称'])
