...
def OnQuote(context, code):
    ...
    if len(position_opt) != 0:
        ...
        #获取k1,k2
        k1 = GetContractInfo(position_opt[1].contract)['行权价格']
        k2 = GetContractInfo(position_opt[0].contract)['行权价格']

        if code_price < min(k1, k2) or code_price > max(k1 ,k2):
            ...
            a = True
            print(str(current_time) + '，触发止损条件，平仓操作')
        elif GetDaystoExpire(opc) == context.offset_day:  # 距离期权到期日一定日期时，平仓操作
            for opt in position_opt:
                OptClosePrice = GetQuote(opt.contract).now
                OptCloseVolume = opt.volume
                order_opt = context.myaccOPT.InsertOrder(opt.contract, BSType.BuyClose, OptClosePrice, OptCloseVolume)
            a = True
            print(str(current_time) + '，触发移仓条件，平仓操作')
        else:
            pass
