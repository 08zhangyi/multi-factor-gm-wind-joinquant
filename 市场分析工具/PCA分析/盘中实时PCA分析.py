...
def get_data(now_date):
    ...
    start_date = w.tdaysoffset(-241, now_date, "").Data[0][0].strftime('%Y-%m-%d')
...
def plot_pca_components(n):
    ...
    fig.tight_layout()
    # plt.show()
    plt.savefig("data\\in_trading\\第"+str(n)+"个主成分的组成系数图.jpg")


# 主成分回报图
def plot_pca_return_values(return_oneday, n):
    # return_oneday为某一日的各行业收益回报，n为计算到第几个PCA（不包括n）
    return_oneday = np.array(return_oneday).reshape((-1, 1)) * 100.0
    ...
    fig.tight_layout()
    # plt.show()
    plt.savefig("data\\in_trading\\主成分回报分布图.jpg")


# 设定计算日期函数和主成分提取参数
NOW_DATE = datetime.datetime.now().strftime('%Y-%m-%d')  # 获取运行时的日期
N = 5
# 读取数据
get_data(NOW_DATE)
df = pd.read_csv('data\\in_trading\\data.csv', index_col=0)
...
# 测试计算每日的收益率分布
w.start()
test_return = w.wsq(','.join(columns), "rt_pct_chg").Data[0]
